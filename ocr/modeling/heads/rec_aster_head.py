  ...
  """
    ...
    def forward(self, x, embed):
        ...
        for i in range(max(lengths)):
            if i == 0:
                y_prev = torch.full([batch_size], fill_value=self.num_classes)
            else:
                y_prev = targets[:, i - 1]
    ...
    def sample(self, x):
        ...
        predicted_ids = torch.concat([predicted_ids, 1], dim=0)
    ...
    def beam_search(self, x, beam_width, eos, embed):
        ...
        x = torch.tile(
            x.unsqueeze(1).permute(1, 0, 2, 3), [beam_width, 1, 1, 1])
        inflated_encoder_feats = x.permute(1, 0, 2, 3).reshape(-1, l, d)

        # Initialize the decoder
        state = self.decoder.get_initial_state(embed, tile_times=beam_width)

        pos_index = (torch.arange(batch_size) * beam_width).reshape(-1, 1)

        # Initialize the scores
        sequence_scores = torch.full([batch_size * beam_width, 1], fill_value=-float('Inf'))
        index = [i * beam_width for i in range(0, batch_size)]
        sequence_scores[index] = 0.0

        # Initialize the input vector
        y_prev = torch.full([batch_size * beam_width], fill_value=self.num_classes)
        ...
        for i in range(self.max_len_labels):
            ...
            scores, candidates = torch.topk(
                sequence_scores.reshape(batch_size, -1),
                beam_width,
                dim=1)

            # Reshape input = (bk, 1) and sequence_scores = (bk, 1)
            y_prev = (candidates % self.num_classes).reshape(batch_size * beam_width)
            sequence_scores = scores.reshape(batch_size * beam_width, 1)

            # Update fields for next timestep
            pos_index = pos_index.expand_as(candidates)
            predecessors = (candidates / self.num_classes + pos_index).long()
            predecessors = predecessors.reshape(batch_size * beam_width, 1)
            state = torch.index_select(
                state, index=predecessors.squeeze(), dim=1)
        ...
        sorted_score, sorted_idx = torch.topk(
            stored_scores[-1].reshape(batch_size, beam_width),
            beam_width)
        ...
        t_predecessors = (sorted_idx + pos_index.expand_as(sorted_idx)).reshape(batch_size * beam_width)
        while t >= 0:
        ...
        re_sorted_idx = (re_sorted_idx + pos_index.expand_as(re_sorted_idx)).reshape(batch_size * beam_width)

        # Reverse the sequences and re-order at the same time
        # It is reversed because the backtracking happens in reverse time order
        p = [
            torch.index_select(step, index=re_sorted_idx, dim=0).reshape(batch_size, beam_width, -1) for step in reversed(p)
        ]
...
class AttentionUnit(nn.Module):
    ...
    def forward(self, x, sPrev):
        ...
        sProj = sProj.unsqueeze(1)  # [b x 1 x attDim]
        ...
        alpha = F.softmax(
            vProj, dim=1)  # attention weights for each sample in the minibatch
        return alpha
...
class DecoderUnit(nn.Module):
    def __init__(self, sDim, xDim, yDim, attDim):
        ...
        self.fc = nn.Linear(sDim, yDim, bias=True)
    ...
    def get_initial_state(self, embed, tile_times=1):
        ...
        if tile_times != 1:
            ...
            trans_state = state.permute(1, 0, 2)
            state = trans_state.reshape(-1, self.sDim)
    ...
    def forward(self, x, sPrev, yPrev):
        ...
        context = torch.matmul(alpha.unsqueeze(1), x).squeeze(1)
        yPrev = yPrev.long()
        yProj = self.tgt_embedding(yPrev)

        concat_context = torch.concat([yProj, context], 1)
        concat_context = concat_context.squeeze(1)
        sPrev = sPrev.squeeze(0)
