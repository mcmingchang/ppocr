...
class TPSSpatialTransformer(nn.Module):
    def __init__(self,
        ...
        target_control_partial_repr = target_control_partial_repr.astype(forward_kernel.dtype)
        forward_kernel[:N, :N] = target_control_partial_repr
        forward_kernel[:N, -3] = 1
        forward_kernel[-3, :N] = 1
        target_control_points = target_control_points.astype(forward_kernel.dtype)
        ...
        Y, X = torch.split(
            target_coordinate, target_coordinate.shape[1], dim=1)
        Y = Y / (self.target_height - 1)
        X = X / (self.target_width - 1)
        target_coordinate = torch.concat(
            [X, Y], dim=1)  # convert from (y, x) to (x, y)
        ...
        target_coordinate_repr = torch.concat(
            [
                target_coordinate_partial_repr, torch.ones(HW, 1),
                target_coordinate
            ],
            dim=1)
