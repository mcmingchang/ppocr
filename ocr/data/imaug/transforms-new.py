...
class ToTensor(BaseTransform):
    ...
    def _apply_image(self, img):
        return torch.from_numpy(np.array(img)).permute(1, 2, 0)
...
class Resize(BaseTransform):
    ...
    def _apply_image(self, img):
        return F.interpolate(img, self.size, mode=self.interpolation)
...
class RandomResizedCrop(BaseTransform):
    ...
    def _apply_image(self, img):
        ...
        if _is_pil_image(img):
            ...
            cropped_img = img[i:i + h, j:j + w, :]
        return F.interpolate(cropped_img, self.size, mode=self.interpolation)
