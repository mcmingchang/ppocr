...
from ocr.utils.save_load import save_model
from ocr.utils.utility import print_dict, AverageMeter
from ocr.utils.logging import get_logger
from ocr.utils.loggers import VDLLogger, WandbLogger, Loggers
from ocr.utils import profiler
...
def check_gpu(use_gpu):
    """
    Log error and exit when set use_gpu=true in torchtorch
    cpu version.
    """
    err = "Config use_gpu cannot be set as true while you are " \
          "using torchtorch cpu version ! \nPlease try: \n" \
          "\t1. Install torchtorch-gpu to run model on GPU \n" \
...
def check_xpu(use_xpu):
    """
    Log error and exit when set use_xpu=true in torchtorch
    cpu/gpu version.
    """
    err = "Config use_xpu cannot be set as true while you are " \
          "using torchtorch cpu/gpu version ! \nPlease try: \n" \
          "\t1. Install torchtorch-xpu to run model on XPU \n" \
          "\t2. Set use_xpu as false in config file to run " \
          "model on CPU/GPU"

    try:
        if use_xpu and not torch.cuda.is_available():
            print(err)
            sys.exit(1)
    except Exception as e:
        pass
...
def preprocess(is_train=False):
    ...
    # check if set use_gpu=True in torchtorch cpu version
    use_gpu = config['Global']['use_gpu']
    check_gpu(use_gpu)

    # check if set use_xpu=True in torchtorch cpu/gpu version
    use_xpu = False
    ...
    device = 'cpu'
    if use_gpu:
        device = 'gpu count:{}'.format(torch.cuda.device_count())
    if use_xpu:
        device = 'xpu'
